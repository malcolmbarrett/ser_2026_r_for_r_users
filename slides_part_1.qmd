...
Numbers got `print()`ed (NB: you do not need to write `print()` if you are just running interactively like this!), then **hand-typed into a Word doc**.
...
- **Inline code** means numbers never go stale:
   ```
   Sample size was n = `r knitr::inline_expr("nrow(births)")`
   ```
